...
class Employee(models.Model):
    ...
    # hr_holidays_ids = fields.One2many('hr.holidays','employee_id',string='Holidays')
    rem_val = fields.Float(string='Remaining Leaves count')
    ...
    def _compute_remaining_leaves(self):
        ...
        print("The remaining leaves of",remaining)
        # print(a)
        for employee in self:
            employee.remaining_leaves = remaining.get(employee.id, 0.0)
            # employee.update({'rem_val': remaining.get(employee.id, 0.0)})
            self.env.cr.execute(
                """update hr_employee set rem_val = %s where id= %s""",
                (remaining.get(employee.id, 0.0), employee.id))

            print ("employeeemployee",employee,employee.rem_val)
        return True
