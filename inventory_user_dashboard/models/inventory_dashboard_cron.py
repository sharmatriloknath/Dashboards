...
class InventoryDashboardCron(models.Model):
    _description = "Cron Detail"
    _name = "inventory.dashboard.cron"
    ...
    def onchange_interval(self):
        model_id = self.env["ir.model"].search([("model", "=", "inventory.user.dashboard")]).id
        ...
        if cron:
            ...
            cron.sudo().create({"name": "update inventory dashboard", "state": "code", "user_id": self.env.user.id,
    ...
    def create(self, values):
        interval = self.env["inventory.dashboard.cron"].search([("create_uid", "=", self.env.user.id)])
        if interval:
            raise ValidationError(_('Time Interval already exist!'))
        return super(InventoryDashboardCron, self).create(values)
